...
pub fn scan_file(
    base_file_path: &str,
) -> Result<(Vec<StarPositionedToken>, StarFileTable), (String, Option<StarPosition>)> {
    let base_file_path_string = base_file_path.to_string();

    let mut file_table: StarFileTable = StarFileTable::new();
    ...
    match scan_and_resolve_processors(
        &"No one".to_string(),
        ...
        Ok(ptokens) => Ok((ptokens, file_table)),
        Err(er) => Err(er),
    }
}
...
pub fn scan(
    ...
    if !file_table.contains_key(&file_id) {
        file_table.insert(file_id, virtual_path.clone());
    }
...
fn scan_and_resolve_processors(
    ...
    let file_id = match file_table.iter().find_map(|(id, path)| if path == &absolute_file_path { Some(*id) } else { None }) {
        Some(id) => id,
        None => {
            *file_counter += 1;
            *file_counter
    ...
    if !file_table.contains_key(&file_id) {
        file_table.insert(file_id, absolute_file_path.clone());
    }

    // ==== COMMON RESOLUTION ====
    if let Err(e) = resolve_processors_common(
        &absolute_file_path,
        file_table,
        file_id,
        &mut ptokens,
...
fn resolve_processors_common(
    ...
    while token_counter < ptokens_len {
        ...
        match tk.token.clone() {
            StarToken::Processor(StarProcessor::Include) => {
                if !file_mode {
                    return Err((
                ...
                match ptokens.get(token_counter + 1).cloned() {
                    Some(next_p_tkn) => {
                        ...
                        {
                            let included_ptokens = match scan_and_resolve_processors(
                                absolute_file_path,
            ...
            StarToken::Processor(StarProcessor::Define) => {
                match ptokens.get(token_counter + 1).cloned() {
                    Some(define_identifier_ptkn) => match define_identifier_ptkn.token {
                        StarToken::Identifier(identifier_string) => {
                            let (macro_head, macro_definition_head_tkns_found) = match ptokens
                                .scan_macro_definition_head(
                                    token_counter + 2,
            ...
            StarToken::Processor(StarProcessor::Once) => match once_set.get(absolute_file_path) {
                Some(_) => {
            ...
            StarToken::Identifier(identifier_string) => {
                if let Some((macro_head, define_sequence)) = macro_table.get(&identifier_string) {
                    let (macro_call_head, head_tokens_quantity_found) = match ptokens
                        .scan_macro_calling_head(token_counter + 1, tk.position.clone())
                    {
                        Ok((head, q)) => (head, q),
                        Err((e, error_pos)) => return Err((e, Some(error_pos))),
                    };
